...
def process_single_frame(frame, yolo_model, predict_kw):
    ...
    global_threat_score = (0.8 * weapon_score) + (0.2 * posture_score)
...
def transcription_worker(audio_queue: queue.Queue, stop_event: threading.Event):
    print("Loading Whisper model...")
    whisper_model = None
    for compute_type in (WHISPER_COMPUTE_TYPE, "int8", "float32"):
        try:
            whisper_model = WhisperModel(WHISPER_MODEL, device=WHISPER_DEVICE, compute_type=compute_type, cpu_threads=0, num_workers=1)
            break
        except Exception:
            pass
    ...
    gemini_client = genai.Client() if use_gemini else None

    buffer = []
    buffer_len = 0
    target_len = int(WHISPER_SAMPLE_RATE * LIVE_CHUNK_SEC)
    
    full_transcript_history = []
    
    print(f"Transcription active. Gemini Trigger Words: {TRIGGER_WORDS}")
    ...
    while not stop_event.is_set():
        try:
            ...
            if buffer_len >= target_len:
                ...
                new_text_parts = [s.text.strip() for s in segments if s.text.strip()]
                ...
                if new_text_parts:
                    ...
                    if gemini_client:
                        ...
                        if any(trigger in lower_text for trigger in TRIGGER_WORDS):
                            context_to_send = " ".join(full_transcript_history[-5:])
                            threading.Thread(target=assess_threat, args=(gemini_client, context_to_send), daemon=True).start()
...
def run_with_audio_av(source: str, yolo_model, args, filter_cls_ids, use_half: bool, device: str, is_general: bool, class_names: dict):
    ...
    if audio_stream and not args.no_audio:
        threading.Thread(target=transcription_worker, args=(audio_queue, audio_stop), daemon=True).start()
            ...
            if packet.stream.type == "video":
                ...
                for frame in packet.decode():
                    try:
                        ...
                        if in_rate != WHISPER_SAMPLE_RATE and in_rate > 0:
                            n_out = int(round(arr.shape[0] * WHISPER_SAMPLE_RATE / in_rate))
